...
- Each mode owns its own provider state machine:
  - Review supports flip, rating, retry-queue-on-`again`, and card flagging (`lib/features/study/presentation/providers/review_provider.dart`, `lib/features/study/presentation/screens/review_mode_screen.dart`).
  ...
  - Recall supports free typing, immediate reveal, self-rating, and in-session retry resolution for missed cards (`lib/features/study/presentation/providers/recall_provider.dart:95-267`, `lib/features/study/presentation/widgets/recall_round_view.dart:33-108`).
  - Fill supports fuzzy evaluation, close-answer accept/reject, retry, hint, and skip-after-retry; completion now ends on the summary surface rather than a separate practice-only branch (`lib/features/study/presentation/providers/fill_provider.dart:123-490`, `lib/features/study/presentation/widgets/fill_feedback_panel.dart:30-157`, `lib/features/study/presentation/screens/fill_mode_screen.dart:145-210`).
  - Match supports grouped boards, mistake counting, combo/stars/timer UI, and completion when all grouped pairs are matched (`lib/features/study/presentation/providers/match_provider.dart:112-374`, `lib/features/study/presentation/screens/match_mode_screen.dart:89-185`).
...
- Missing docs-defined board fail/retry contract in match mode. The docs say failed items or boards should remain retry-pending until resolved (`lib/features/study/docs/03_session_lifecycle.md:49-60`, `lib/features/study/docs/05_execution_rules.md:54-73`). Recall now keeps missed cards in-session until the retry is resolved, but match still completes once all grouped pairs are eventually matched and never declares a board-level fail or retry-pending state (`lib/features/study/presentation/providers/match_provider.dart:170-260`).
...
## 5. Implemented but not specified in docs
...
- The docs say failed items should retry within the mode (`lib/features/study/docs/03_session_lifecycle.md:49-60`), yet the remediation section also allows other options such as an easier mode or a future remedial session (`lib/features/study/docs/05_execution_rules.md:66-73`). Recall now follows the in-session retry reading, while match still relies on eventual grouped-board completion because the docs do not define a board-level failure threshold precisely enough to force a narrower behavior.
